...
export function PreviewPanel({
  ...
  appTranslations,
  t,
  styleWrapper = "preview-panel border border-gray-200 shadow-sm rounded-xl",
  stylePage = "cv-page",
}: PreviewProps) {
  const Template = templateMap[state.template];

  const { pages, measurer } = useTemplatePages(Template, state, t);

  const totalPages = pages.length;
  const [currentPage, setPageState] = useState<number>(0);
  ...
  return (
    <div className={styleWrapper}>
      ...
      <div className="relative min-h-0 min-w-0 flex-1 overflow-hidden">
        ...
        >
          <div
            className={`flex min-h-full min-w-full p-10 ${
              zoom > 1
